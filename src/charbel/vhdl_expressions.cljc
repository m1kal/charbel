...
(defn max-width [args]
  (cond
    ...
    :else (reduce (fn [acc elem] (str "maximum(" acc ", " elem ")"))
...
(defmethod complex-expression :select [[_ & b] env]
  {:result (str (:result (expression (first b) env))
                "(" (second b) (if (= 2 (count b)) ")" (str " downto " (last b) ")")))
...
(defmethod complex-expression :+ [[_ & r] env]
  (let [e (map #(expression % env) r)]
    {:result (s/join " + " (map (comp u :result) e)) :width (width-sum [(max-width (map :width e)) 1])}))

(defmethod complex-expression :- [[_ & r] env]
  (let [e (map #(expression % env) r)]
    {:result (s/join " - " (map (comp u :result) e)) :width (max-width (map :width e))}))

(defmethod complex-expression :* [[_ & r] env]
  (let [e (map #(expression % env) r)]
    {:result (s/join " * " (map (comp u :result) e)) :width (width-sum (map :width e))}))

(defmethod complex-expression :inc [[_ x] env]
  (let [e1 (expression x env)]
    {:result (str (u (:result e1)) " + 1") :width (width-sum [1 (:width e1)])}))

(defmethod complex-expression :dec [[_ x] env]
  (let [e1 (expression x env)]
    {:result (str (u (:result e1)) " - 1") :width (:width e1)}))

(defmethod complex-expression :bit-and [[_ & r] env]
  (let [e (map #(expression % env) r)]
    {:result (s/join " and " (map :result e)) :width (max-width (map :width e))}))

(defmethod complex-expression :bit-or [[_ & r] env]
  (let [e (map #(expression % env) r)]
    {:result (s/join " or " (map :result e)) :width (max-width (map :width e))}))

(defmethod complex-expression :bit-xor [[_ & r] env]
  (let [e (map #(expression % env) r)]
    {:result (s/join " xor " (map :result e)) :width (max-width (map :width e))}))

(defmethod complex-expression :bit-not [[_ x] env]
  (let [e1 (expression x env)]
    {:result (str "not " (:result e1)) :width (:width e1)}))

(defmethod complex-expression :if [[_ x y z] env]
  (let [e1 (expression x env) e2 (expression y env) e3 (expression z env)]
    {:result (str (:result e2) " if " (:result e1) " else " (:result e3))
  ...
  (let [e (expression expr env)]
    (if (empty? args)
      ...
      (let [r (expression (cons :cond args) env)]
        {:result (str (:result e) " if " (:result (expression cond env)) " else " (:result r))
